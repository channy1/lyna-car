...
<div class="portlet light bordered">
    ...
    <div class="row">
        ...
        $view_id = $_GET["id"]; 
        $query="SELECT * FROM tbl_accessories_rental_img WHERE acc_id='$view_id'";
        $result = $connect->query($query);
        if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()){
        ?>
        <div class="col-md-2">
            <a href="ajax_delete.php?delete_id=<?php echo $row['img_id']; ?>&acc_id=<?php echo $row['acc_id']; ?>" onclick="return confirm('Do you want to delete!!')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-remove"></i>Delete</a>
