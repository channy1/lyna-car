                    ...
                    if(isset($_POST['btn_search'])){
                        ...
                        if(@$_POST['txt_employee']!=""){
                            $v_employee = @$_POST['txt_employee'];
                                ...
                                WHERE A.q_date_sign BETWEEN '$v_s_date' AND '$v_e_date' AND A.q_customer='$v_employee' AND A.q_menu_type=8";
                                $result = $connect->query($v_sql);
                                
                        }else{
                            $v_sql = "SELECT * FROM tbl_quotation AS A 
                                left JOIN tbl_customer AS B ON B.cus_id=1
                                left join tbl_quotation_type_menu AS C ON A.q_menu_type=C.qtm_id WHERE A.q_date_sign BETWEEN '$v_s_date' AND '$v_e_date' AND A.q_menu_type=8";
                                $result = $connect->query($v_sql);
                        }
                    }else{
                        $v_sql = "SELECT * FROM tbl_quotation AS A 
                        left JOIN tbl_customer AS B ON B.cus_id=1
                        left join tbl_quotation_type_menu AS C ON A.q_menu_type=C.qtm_id WHERE A.q_menu_type=8";
                        $result = $connect->query($v_sql);
                    }
