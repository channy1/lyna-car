...
// echo "kemun";
  ...
  if(!empty($_GET)){
      ...
      $v_sql1 = "SELECT * FROM tbl_pickup_drop_off where air_id = '$p_id'";
      $result1 = $connect->query($v_sql1);

      if ($result1->num_rows > 0) {
          while($row1 = $result1->fetch_assoc()) {
              echo $provice_id = $row1['provice_id'];
              exit;
          }
      }
  }else{
...
<div  style="width:100%;">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                  <?php
                                 $id=@$connect->real_escape_string($_POST['ve_id_ajax']);
                                 $v_sql = "SELECT * FROM tbl_pickup_drop_off 
                                 INNER JOIN tbl_province
                                 ON tbl_pickup_drop_off.pro_from_id = tbl_province.pv_id
                               ...
                               //echo $v_sql;
                                 ...
                                 while($row = $result->fetch_assoc()) {
                                  ...
                                  <tr>
                                    ...
                                    ?>
                                    <td style="text-align:right;"><?php echo number_format($row['distant'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['rate_one'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['discount_one'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['one_way'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['rate_two'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['discount_two'],2); ?></td>
                                    <td style="text-align:right;"><?php echo number_format($row['round_trip'],2); ?></td>
                                    <td><?php echo $row['note']; ?></td>
                                    <td>
                                      <a href="delete_pickup_port.php?delete_id=<?php echo $row['air_id']; ?>" onclick="return confirm('Do you want to delete!!')" class="delete" title="" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-remove"></i></a>
                                    </td>
                                  </tr>
                                <?php
                                   // }
